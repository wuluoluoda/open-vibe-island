...
<p align="center">
  <a href="https://github.com/wuluoluoda/open-island/releases/latest"><img src="https://img.shields.io/github/v/release/wuluoluoda/open-island?style=flat-square&label=release&color=blue" alt="最新发布"></a>
...
<p align="center">
  <a href="https://github.com/wuluoluoda/open-island/releases">发布版本</a> ·
  <a href="#快速开始">快速开始</a> ·
...
Open Island 基于 [Octane0411/open-vibe-island](https://github.com/Octane0411/open-vibe-island) 修改，并按 GNU General Public License v3.0 发布。这个仓库由 [wuluoluoda](https://github.com/wuluoluoda) 作为独立 GPLv3 项目维护，重点面向高频本地 Codex 工作流。
...
可以从 [GitHub Releases](https://github.com/wuluoluoda/open-island/releases) 下载当前 DMG。除非发布说明另有标注，本地构建可能未签名，下载后 macOS Gatekeeper 可能需要右键**打开**，或手动移除 quarantine。
...
- **开源** — GPL v3，修改它、审计它，并在相同许可证下发布你自己的版本
...
当前 DMG 可从 [wuluoluoda/open-island releases](https://github.com/wuluoluoda/open-island/releases) 下载。除非发布说明另有标注，构建可能未签名。
...
```
我在使用 Open Island (https://github.com/wuluoluoda/open-island) 时遇到了问题。
...
- **Codex CLI** — Hook 集成。受管安装器默认只安装 `SessionStart`、`UserPromptSubmit` 和 `Stop`，以保持终端输出低噪声。Open Island 可以解析手动配置的 `PreToolUse` / `PostToolUse` 等更细粒度 Codex hook 事件，但这些事件不属于默认受管安装范围。Codex 文件编辑可能走内部 apply-patch 路径，因此不应把文件编辑审批描述为稳定的 `PreToolUse` 覆盖能力。从本地 rollout 文件读取 5 小时和 7 天 account usage windows。支持从设置窗口或 CLI 安装/卸载受管 hooks。
